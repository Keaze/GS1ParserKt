...
class GS1ScannerTest : ShouldSpec({
    ...
    context("isGS1Format") {
        ...
        should("return true if barcode starts with fnc1 but is not only fnc1"){
    ...
    context("parse simple GS1 Barcode"){
        should("return an error message on invalid ai"){
            val barcode = fnc1 + "XXXXX"
            scanner.parse(barcode) shouldBe ResultKt.failure(Gs1ParseErrors.AiNotFound)
        }

        should("barcode with valid ai should have the correct value"){
